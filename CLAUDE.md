...
- [x] **Décisions app (27/07/2026)** : on reste sur **Navigator** pour l'app transporteur (pas de custom) ; les deux interfaces custom se feront en **Flutter pur**, pas FlutterFlow (l'éditeur visuel n'a pas d'intérêt dès lors que c'est Claude Code qui écrit le code). Reste à faire : installer/tester Navigator en pratique (jamais fait), et une vérification technique ciblée (déploiement Flutter web, pattern de polling) avant de démarrer le développement des deux interfaces.
