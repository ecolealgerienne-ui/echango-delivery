...
⚠️ **`test-wilaya.sh` porte ses propres témoins, et c'est ce qui le rend utile.** ⚠️ **Le CONDUCTEUR ne filtre plus par wilaya depuis le 09/09/2026** — point d'ancrage + rayon, filtre spatial Fleetbase (`test-filtre-rayon.sh`). Le champ `province` sur la commande sert maintenant la zone de service ENTREPRISE (`test-zone-entreprise.sh`, `test-opportunites-filtre.sh`), qui reste une liste de wilayas ; une course qui ne la transporte pas est **invisible au filtre entreprise**, sans erreur ni journal — une liste simplement plus courte. Trois contrôles, chacun avec son cas négatif dans le même passage :
...
⚠️ **`audit_ecrans_test.dart` porte un second scénario (04/08/2026) : « rendue au réseau » ≠ « écartée ».** Refuser une OPPORTUNITÉ diffusée l'écarte pour ce conducteur (`releasedToPool:false`) ; RENDRE une course qu'on lui a CONFIÉE la renvoie au réseau (`releasedToPool:true`) et prévient le commerçant. Même bouton, même tiroir — **seul le message change**, ce qu'un `curl` ne voit pas. Le test joue les **deux branches** (course confiée dans « En cours », opportunité dans « Opportunités ») et vérifie que chaque geste affiche SON message et pas l'autre (témoin positif + négatif). Le décor pose la course confiée par **ciblage de favori** (`provision-app-parcours.sh` : `targetFavouriteUuid`, prix distinctif 4444, idempotente). ⚠️ **La seule assertion du dépôt sur un message traduit** — la distinction est textuelle, rien de structurel ne sépare les deux SnackBars ; on calcule les deux messages via le traducteur de l'app pour sa **locale courante**, donc le test tient en FR comme en AR (le défaut que vise la règle « jamais par un texte traduit » du harness est un littéral figé, pas une lecture du traducteur).

⚠️ **`audit_ecrans_test.dart` porte quatre scénarios (04/08/2026) — les deux derniers ajoutés d'un coup.** **(3) La zone du conducteur fait l'aller-retour serveur** : il pose un **point de base (lat/lng) + un rayon** sur son profil, quitte l'écran et y revient — la phrase d'état **relue** du serveur (« 20 km ») prouve la persistance, pas un écho des champs ; le test **retire** le point de base en fin, sinon un rayon de 20 km autour de Blida filtrerait les opportunités des autres parcours. **(4) L'entreprise réclame puis affecte** : « Courses libres » → « Prendre » → « Mes courses » → « Désigner » → tiroir de conducteurs. Deux appuis structurels y sont nécessaires : `find.*.hitTestable()` borne chaque finder à l'**onglet visible** (le `TabBarView` à 4 onglets peuple l'arbre des 3 autres) et à la **fiche par-dessus l'accueil** (une route poussée laisse la précédente dans l'arbre) ; et le décor **rattache un conducteur à la flotte** et **SUPPRIME** (pas annule — une annulée reste affichée) les courses de « Mes courses », pour que la course réclamée y soit seule et identifiable.

⚠️ **Et ces quatre scénarios ont trouvé un vrai défaut produit — `OrderState` notifié après `dispose`.** Le conducteur interroge le BFF en boucle (`DriverPresenceState._pollTimer` → `loadOrders`) ; le timer est bien annulé au `dispose`, mais `cancel()` n'interrompt pas un `loadOrders()` **déjà parti** attendre l'endpoint (~9 s) : à son retour, il notifiait un `OrderState` détruit — « A OrderState was used after being disposed », le défaut « notifier une classe d'état détruite » du 03/08. Corrigé par un garde `_notify()` (un état disposé ne réveille plus personne). **Invisible aux tests plus courts** : c'est la suite de quatre scénarios enchaînés qui l'a exposé, exactement le genre de défaut que seul le jeu à l'écran révèle.
