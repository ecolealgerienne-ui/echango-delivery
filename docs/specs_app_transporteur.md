...
**Conséquence pour §11 (temps réel)** : voir la décision correspondante ci-dessous — le canal FCM/APN natif de Fleetbase (`OrderPing`) cible le `Driver` Fleetbase, pas le compte Echango. Le BFF doit donc, à l'enregistrement du jeton push par l'app, l'écrire côté Fleetbase via son compte de service — un pont nécessaire entre les deux couches d'identité. **Correction (28/07/2026)** : le jeton push ne se pose **pas** sur l'enregistrement `Driver` lui-même (aucune colonne fcm/apn sur ce modèle) — confirmé en lisant le code source public `fleetbase/fleetops` : `Driver::routeNotificationForFcm()`/`routeNotificationForApn()` lisent une relation `devices()` (`HasMany` vers `UserDevice`, jointure sur `user_uuid`, pas `driver_uuid`), un modèle séparé du package `core-api`. Le BFF doit donc écrire un enregistrement `UserDevice` (route `int/v1/user-devices`, macro `fleetbaseRoutes()`), pas patcher le `Driver`. Implémenté dans `backend/bff/src/fleetbase/fleetbase-api.client.ts::upsertDriverDeviceToken()` et **validé de bout en bout par test réel le 28/07/2026** (`scripts/test-driver-auth.sh`, instance Fleetbase locale) : le `UserDevice` est bien créé et son uuid remonté. Reste ouvert : upsert vs doublon sur appels répétés avec le même jeton. Détail dans `docs/journal_implementation_bff.md` §5.1.
