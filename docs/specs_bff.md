...
- `GET /flotte/drivers/positions` — **positions groupées de tous les drivers de la Fleet en un seul appel**, ajouté (un appel par driver ne scale pas pour l'écran carte, identifié comme le plus exigeant du spike FlutterFlow)
...
**Optimisation à documenter pour la recherche par `facilitator_uuid`** (`docs/rapports_specs_bff/01_architecture.md` §4) : ne pas "fetch-all puis filtrer en mémoire" — Fleetbase supporte un filtrage générique par paramètre de requête côté serveur (`?facilitator_uuid=...`, via `HasApiControllerBehavior`). Pousser ce filtre en base pour éviter de rapatrier tout l'historique de l'Organization à chaque appel (volume, latence, exposition transitoire de données cross-tenant dans les logs/traces du BFF) — **et**, en défense en profondeur, revalider systématiquement chaque ligne retournée côté BFF avant de la renvoyer au client (précédent concret : le bug de format `customer_type` a montré qu'on ne peut pas faire confiance aveuglément au filtrage Fleetbase, même serveur).
...
- **Bearer-only, jamais de cookie de session.** À figer explicitement dès maintenant : la techno de l'interface "petite flotte" n'est pas encore arrêtée (spike FlutterFlow à venir, qui peut produire une app web) — poser cette contrainte maintenant neutralise structurellement le risque CSRF sans avoir à y revenir plus tard.
