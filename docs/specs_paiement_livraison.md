...
4. ~~**Les frais de livraison** sont-ils encaissés en même temps que la marchandise, comme le font les transporteurs algériens, ou réglés à part ?~~ **Tranché le 30/07/2026 — c'est le commerçant qui décide, par commande.** Une case « les frais de livraison sont inclus » à la création : cochée, le destinataire règle un montant unique et le commerçant supporte la course ; décochée, la rémunération du transporteur est **ajoutée** au prix de la marchandise et réclamée au destinataire en plus. Dans les deux cas `meta.cod_amount` porte **ce qui est réclamé à la porte, livraison comprise** — un seul sens pour tous ses lecteurs (montant annoncé au transporteur, `expectedAmount` figé, refus de percevoir plus que dû, plafond de dette) — et `meta.cod_goods_amount` conserve le prix de la marchandise tel que saisi. Le règlement, lui, ne change pas : `dette = perçu − rémunération` dans les deux cas (§ *Règlement net*, journal §17). L'addition est faite **par le serveur, une seule fois**, à l'écriture ; le formulaire en montre le résultat avant validation, et refuse une livraison à la charge du destinataire sans rémunération connue — sans elle, il n'y a pas de montant à réclamer.
