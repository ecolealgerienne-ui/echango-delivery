...
- [ ] **Le transporteur choisit ce qu'il voit : wilaya d'abord, rayon autour (décidé ET branché le 02/08/2026 — sauf les notifications)**
  ...
  ⚠️ **RÉVISÉ le 09/09/2026 — la wilaya est retirée du côté conducteur.** Elle imposait une liste des 58 à tenir accordée avec l'administration algérienne et couplait le produit à un pays. La zone conducteur est désormais **un point d'ancrage (lat/lng, choisi — pas la position GPS vive) + un rayon**. ⚠️ **Le filtre est calculé EN MÉMOIRE** (`pickupWithinZone` remplace `zoneAllows`/`zoneAllowsPickup`) : `GET /v1/orders?nearby` **n'accepte pas de rayon par requête** — mesuré, il n'applique que `company.options.fleetops.adhoc_distance` (6 km d'org). Le parcours `/int/v1` (`fetchEveryOrder`) reste donc en place (« c'est un coût, pas un défaut »), lancé seulement si un point d'ancrage existe. Sans point d'ancrage, la liste des opportunités est **vide + `anchorMissing`** (l'écran invite à en poser un). Banc : `scripts/test-filtre-rayon.sh`, mutation `pickupWithinZone → return true`. ⚠️ **La zone de service ENTREPRISE reste une liste de wilayas** (`fleet-zone.ts`, `test-zone-entreprise.sh`) — hors périmètre. Le reste de cette section décrit l'ancienne conception ; elle vaut comme trace de décision.
