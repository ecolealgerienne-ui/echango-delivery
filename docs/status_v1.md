...
- [ ] **Le transporteur choisit ce qu'il voit : wilaya d'abord, rayon autour (décidé ET branché le 02/08/2026 — sauf les notifications)**
  ...
  ⚠️ **RÉVISÉ le 09/09/2026 — la wilaya est retirée du côté conducteur.** Elle imposait une liste des 58 à tenir accordée avec l'administration algérienne et couplait le produit à un pays. La zone conducteur est désormais **un point d'ancrage (lat/lng, choisi — pas la position GPS vive) + un rayon**, et le **filtrage spatial est délégué à Fleetbase** (`GET /v1/orders?nearby&radius`, `ST_Distance_Sphere` sur son index) : le BFF ne calcule plus de distance sur ce chemin. Sans point d'ancrage, la liste des opportunités est **vide + `anchorMissing`** (l'écran invite à en poser un). `pickupWithinZone` remplace `zoneAllows`/`zoneAllowsPickup` comme revérification en mémoire et point de mutation des bancs (`scripts/test-filtre-rayon.sh`). ⚠️ **La zone de service ENTREPRISE reste une liste de wilayas** (`fleet-zone.ts`, `test-zone-entreprise.sh`) — hors périmètre de ce changement. Le reste de cette section décrit l'ancienne conception ; elle vaut comme trace de décision.
