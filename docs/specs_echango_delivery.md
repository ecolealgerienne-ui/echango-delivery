...
5. ~~Tester le mode `adhoc`~~ **✅ Fait** (26/07/2026, voir §3.2) : pipeline serveur confirmé de bout en bout (recherche géospatiale, event, queue, notification déclenchée sans erreur). Reste seulement la réception réelle par un client (Navigator), déjà liée à l'action différée correspondante. Le comportement du retry à 4 minutes (`fleetops:dispatch-adhoc`) reste à observer sur la durée — non testé.
