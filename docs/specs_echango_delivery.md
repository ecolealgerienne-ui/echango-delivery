...
- **Reste à tester** : le rattachement d'un Contact comme "personnel" d'un `Vendor` (`vendorPersonnel`) pour valider le scoping côté persona "petite flotte"/commerçant-avec-Vendor, et la création d'une vraie commande visible par ce compte pour boucler la validation de bout en bout.
...
2. **Spike `fleetbase/customer-portal`** — **✅ pour l'essentiel fait et concluant** (26/07/2026, voir §3.1) : installation, création d'un compte `Contact`, login réel, scoping des commandes confirmé vide/isolé. Reste à tester : le rattachement via `Vendor`/`vendorPersonnel` (persona petite flotte), et une commande réelle visible de bout en bout. La décision "le BFF s'appuie sur `customer-portal-api`" est maintenant fortement recommandée plutôt qu'hypothétique — sous réserve de la maturité v0.0.x à surveiller en continu.
