...
- **`Color.withValues` retiré.** Il exige Flutter 3.27 alors que le pubspec déclare `>=3.20`. La couleur de l'icône portait déjà la distinction ; la pastille teintée n'apportait rien qui justifie une contrainte de version. ⚠️ **Note du 01/08/2026** : la borne citée ici était fausse (elle est passée à `>=3.24.0`, déduite de la contrainte Dart du même fichier), mais **ce retrait-ci tient sans changement** — 3.27 reste au-dessus de 3.24. Contrairement aux trois autres API écartées sur ce motif, celle-ci l'était à bon droit.
