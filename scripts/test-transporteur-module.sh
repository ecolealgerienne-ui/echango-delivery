...
if [ "$N_ADHOC" -gt 0 ]; then
  ...
  LEAK=$(echo "$RESP" | jq -c '[.adhoc[0].payload.dropoff // {} | .. | objects
        | with_entries(select(.key | test("street|latitude|longitude|contact_name|phone|email")))
        | select(length > 0)]')
  [ "$LEAK" = "[]" ] \
    || fail "FUITE : données du destinataire sur une adhoc non réclamée" "$LEAK"

  # Le client peut aussi fuir par la racine de la commande, hors du point de
  # livraison — c'est la porte de côté que l'expurgation doit fermer aussi.
  LEAK=$(echo "$RESP" | jq -c '[.adhoc[0].customer // empty]')
  [ "$LEAK" = "[]" ] \
    || fail "FUITE : objet client présent à la racine d'une adhoc" "$LEAK"
  pass "adhoc — client réduit à sa commune (liste)"
  ...
  LEAK=$(echo "$DETAIL" | jq -c '[.payload.dropoff // {} | .. | objects
        ...
        | select(length > 0)]')
  [ "$LEAK" = "[]" ] \
    || fail "FUITE : le détail expose les données du destinataire" "$LEAK"

  LEAK=$(echo "$DETAIL" | jq -c '[.customer // empty]')
  [ "$LEAK" = "[]" ] \
    || fail "FUITE : objet client présent à la racine du détail" "$LEAK"
